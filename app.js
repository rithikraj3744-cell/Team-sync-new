...
function renderAdmin(){
  ...
  if(CU.role==='captain'){
    ...
    if(captainLeave && captainLeave.active){
      ...
      leaveStatusHTML = `
        <div class="info-pill" style="margin-bottom:16px;background:${leaveActive?'rgba(110,231,183,.08)':'rgba(252,211,77,.07)'};border-color:${leaveActive?'rgba(110,231,183,.3)':'rgba(252,211,77,.25)'};color:${leaveActive?'var(--acc5)':'var(--acc4)'}">
          ${leaveActive?'🟢 <strong>Access currently delegated</strong>':'🟡 <strong>Leave scheduled (not active yet)</strong>'} — <strong>${vcName}</strong> has captain-level access<br>
          <span style="font-size:.75rem;color:var(--t3);margin-top:3px;display:block">📅 ${typeLabel} · Applied: ${captainLeave.appliedAt||''}</span>
        </div>
    ...
    <div class="card" style="margin-bottom:18px;position:relative;overflow:hidden">
      ...
      <div class="ct"><span class="cd" style="background:var(--acc4)"></span>Captain Absence — Delegate Access to Vice Captain</div>
      <div class="info-pill ip-info" style="margin-bottom:16px">👑 When you're absent, the <strong>Vice Captain</strong> gets full captain-level access. They can manage members, tasks, admin, and all features. Access is <strong>only granted by you</strong> and can be revoked early anytime.</div>
      ${viceMember ? '' : `<div class="info-pill ip-warn" style="margin-bottom:16px">⚠ No Vice Captain found. Add a member with the "Vice Captain" role first.</div>`}
      ${leaveStatusHTML}
      ...
      <!-- Single day fields -->
      <div id="leave-single-fields" class="fg3" style="margin-bottom:14px">
        <div class="field"><label>Date *</label><input type="date" id="leave-single-date" min="${new Date().toISOString().split('T')[0]}"></div>
        <div class="field"><label>Absent From (time)</label><input type="time" id="leave-single-start" value="09:00"></div>
        <div class="field"><label>Absent Until (time)</label><input type="time" id="leave-single-end" value="18:00"></div>
      </div>

      <!-- Date range fields -->
      <div id="leave-range-fields" class="fg" style="margin-bottom:14px;display:none">
      ...
      <div style="padding:12px;background:var(--s2);border:1px solid var(--b1);border-radius:var(--r8);margin-bottom:14px;font-size:.82rem;color:var(--t2)">
        👤 Access will be granted to: <strong style="color:var(--acc2)">${viceMember.name}</strong> <span class="badge b-vice" style="margin-left:6px">Vice Captain</span>
      </div>
  ...
  if(isActingCaptain){
    leaveCardHTML = `<div class="info-pill" style="margin-bottom:18px;background:rgba(110,231,183,.08);border-color:rgba(110,231,183,.3);color:var(--acc5)">
