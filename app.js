...
const DEPTS = ['Computer Science & Engineering','Information Technology','Electronics & Communication',
  ...
  'AI & Data Science','Cyber Security','MBA / Management','Biotechnology','Other'];
...
const DEPT_SHORT = {
  ...
  'AI & Data Science':'AIDS','Cyber Security':'CYBER',
...
const DEPT_COLOR = {
  ...
  'AI & Data Science':'#22d3ee','Cyber Security':'#f472b6',
...
/* ════════════════════════════════════════════════════════════
   ...
   Each team's data lives under teams/{TEAM_CODE}/{collection}
...
/* ── Multi-team identity ──
   Every team gets a unique Team Code (Firestore doc id under the top-level
   "teams" collection) plus a Passcode chosen by whoever creates the team.
   All of a team's data lives in subcollections under teams/{TEAM_CODE}/... */
let TEAM_CODE     = null;
let TEAM_NAME     = null;
let TEAM_PASSCODE = null;
...
/* Reference to a collection scoped to the current team */
function teamCol(col){
  return db().collection('teams').doc(TEAM_CODE).collection(col);
}
...
async function loadCol(col, orderField){
  ...
  } catch(e){ console.warn('loadCol error', col, e); return []; }
}
...
async function reloadData(){
  ...
  try {
    const snap = await teamCol('settings').doc('captainLeave').get();
...
async function saveCaptainLeave(leaveData){
  await teamCol('settings').doc('captainLeave').set(leaveData);
  captainLeave = leaveData;
}

async function clearCaptainLeave(){
  await teamCol('settings').doc('captainLeave').delete();
  captainLeave = null;
}
...
async function saveDoc(col, data){
  ...
  await teamCol(col).doc(docId).set(clean);
}

/* Delete a document */
async function delDoc(col, id){
  await teamCol(col).doc(String(id)).delete();
}
...
/* ════════════════════════════════════════════════════════════
   LOADING / NETWORK ERROR UI  (shared by team-select + boot flow)
════════════════════════════════════════════════════════════ */
function showLoader(){
  if(document.getElementById('net-loading')) return;
...
async function init(){
  ...
  let savedCode = null;
  try { savedCode = localStorage.getItem('ts-team-code'); } catch(e){}
  ...
  try {
    const snap = await Promise.race([
      db().collection('teams').doc(savedCode).get(),
      new Promise((_,rej)=>setTimeout(()=>rej(new Error("timeout")),8000))
    ]);
    if(!snap.exists){
      try { localStorage.removeItem('ts-team-code'); } catch(e){}
      removeLoader();
      showTeamScreen();
      return;
    }
    const data = snap.data();
    TEAM_CODE = savedCode; TEAM_NAME = data.name; TEAM_PASSCODE = data.passcode;
  } catch(e){
  ...
  await proceedAfterTeamSelect();
}

/* Runs once TEAM_CODE is known: loads that team's data, then routes to
   Captain Setup (no captain yet), restored session, or the Login screen. */
async function proceedAfterTeamSelect(){
  showLoader();
  try {
    await Promise.race([
      reloadData(),
...
/* ════════════════════════════════════════════════════════════
   TEAM SELECT — Create a new team or Join an existing one
════════════════════════════════════════════════════════════ */
const TEAM_CODE_CHARS = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789'; // no 0/O/1/I — avoids confusion

function genTeamCodeRaw(){
  let s='';
  for(let i=0;i<6;i++) s += TEAM_CODE_CHARS[Math.floor(Math.random()*TEAM_CODE_CHARS.length)];
  return s;
}

async function genUniqueTeamCode(){
  for(let i=0;i<8;i++){
    const code = genTeamCodeRaw();
    try {
      const snap = await db().collection('teams').doc(code).get();
      if(!snap.exists) return code;
    } catch(e){ /* try again */ }
  }
  // extremely unlikely fallback — append a time-based suffix to guarantee uniqueness
  return genTeamCodeRaw() + Date.now().toString(36).slice(-2).toUpperCase();
}

function showTeamScreen(){
  document.getElementById('team-screen').style.display='flex';
  teamTabSwitch('create');
}

function teamTabSwitch(which){
  ...
  const tab=document.getElementById('tt-'+which), pane=document.getElementById('team-pane-'+which);
  if(tab) tab.classList.add('active');
  if(pane) pane.classList.add('active');
}

function resetTeamData(){
  members=[]; messages=[]; reports=[]; roadmaps=[]; hackathons=[];
  leetcodeStats=[]; dailyTasks=[]; domains=[]; captainLeave=null; CU=null;
  try{ sessionStorage.removeItem('tsync_uid'); sessionStorage.removeItem('tsync_tab'); }catch(e){}
}

async function createTeam(){
  const name = v('team-name');
  const pass = v('team-pass');
  const pass2 = v('team-pass2');
  const err = document.getElementById('team-create-err');
  err.textContent='';
  if(!name){ err.textContent='Team name is required.'; return }
  if(!pass||pass.length<4){ err.textContent='Passcode must be at least 4 characters.'; return }
  if(pass!==pass2){ err.textContent='Passcodes do not match.'; return }
  err.textContent='Creating team…';
  try{
    const code = await genUniqueTeamCode();
    await db().collection('teams').doc(code).set({code, name, passcode:pass, createdAt:today()});
    resetTeamData();
    TEAM_CODE=code; TEAM_NAME=name; TEAM_PASSCODE=pass;
    try{ localStorage.setItem('ts-team-code', code); }catch(e){}
    err.textContent='';
    document.getElementById('created-team-code').textContent=code;
    teamTabSwitch('created');
  }catch(e){
    console.error(e);
    err.textContent='Failed to create team — check your connection / Firebase config.';
  }
}

function copyTeamCode(){
  const code=document.getElementById('created-team-code').textContent;
  navigator.clipboard.writeText(code).then(()=>toast('Team code copied!')).catch(()=>toast('Copy failed — note it down manually','err'));
}

function proceedToCaptainSetup(){
  document.getElementById('team-screen').style.display='none';
  document.getElementById('setup-screen').style.display='flex';
}
...
async function joinTeam(){
  ...
  const pass = v('join-pass');
  const err = document.getElementById('team-join-err');
  err.textContent='';
  if(!code||!pass){ err.textContent='Enter both the Team Code and Passcode.'; return }
  err.textContent='Checking…';
  try{
    const snap = await db().collection('teams').doc(code).get();
    if(!snap.exists){ err.textContent='Team not found — check the code and try again.'; return }
    const data = snap.data();
    if(String(data.passcode)!==pass){ err.textContent='Incorrect passcode.'; return }
    resetTeamData();
    TEAM_CODE=code; TEAM_NAME=data.name; TEAM_PASSCODE=data.passcode;
    try{ localStorage.setItem('ts-team-code', code); }catch(e){}
    err.textContent='';
    document.getElementById('team-screen').style.display='none';
    await proceedAfterTeamSelect();
...
function changeTeam(){
  if(!confirm("Switch to a different team? You'll need that team's Code and Passcode to continue."))return;
  try{ localStorage.removeItem('ts-team-code'); }catch(e){}
...
function showLogin(){
  ...
  const tn=document.getElementById('login-team-name');
  if(tn) tn.textContent = TEAM_NAME ? `${TEAM_NAME} · Code: ${TEAM_CODE}` : '';
  buildLoginGrid();
}
...
function renderDash(){
  ...
  document.getElementById('st-dept').textContent = [...new Set(members.map(m=>m.dept))].length;
  document.getElementById('st-msg').textContent  = messages.length;
  document.getElementById('st-rep').textContent  = reports.length;
  document.getElementById('st-rm').textContent   = roadmaps.length;
  if(document.getElementById('st-hk')) document.getElementById('st-hk').textContent = hackathons.length;
  ...
  if(!members.length){
    list.innerHTML=emptyState('👥','No members yet');return;
  }
  // Sort: captain first, then by role, then name
  const order=['captain','vice','manager','strategist','member'];
  const sorted=[...members].sort((a,b)=>{
    const ra=a.role==='team_leader'?'member':a.role;
    const rb2=b.role==='team_leader'?'member':b.role;
    return order.indexOf(ra)-order.indexOf(rb2)||a.name.localeCompare(b.name);
  });
  list.innerHTML=sorted.map(m=>mrHTML(m)).join('');
}
  ...
  if(canView){
    ...
    if(!allReports.length){html+=emptyState('📄','No reports submitted yet');
      ...
      html+=allReports.map(r=>{
        ...
        const isBinary=r.fileData&&r.fileData.length>0;
...
function submitReport(){
  ...
  reader.onload=async function(e){
    const rep={id:Date.now(),uid:Number(CU.id),uname:CU.name,title:title||file.name,
      content: isBinary ? '' : e.target.result,
      fileData: isBinary ? e.target.result : null,
      fileName: file.name,
      fileType: ext,
      time:nowStr()};
...
function downloadReport(id){
  ...
  if(!r)return;
  if(r.fileData){
    // Binary file (pptx/docx) stored as base64 dataURL
    const a=document.createElement('a');
    a.href=r.fileData;
    a.download=r.fileName||('report-'+id+'.'+(r.fileType||'bin'));
    a.click();
  } else if(r.content){
    const blob=new Blob([r.content],{type:'text/plain'});
    const url=URL.createObjectURL(blob);
    const a=document.createElement('a');
    a.href=url;a.download=r.fileName||(r.title||'report')+'.txt';a.click();
...
/* ════════════════════════════════════════════════════════════
   LEETCODE TRACKER
   Any member can update their own entry; Captain/Vice can edit all
════════════════════════════════════════════════════════════ */
...
function renderLeetCode(){
  ...
  <div class="card" style="margin-bottom:22px">
    <div class="ct"><span class="cd" style="background:#f89f1b"></span>${lcEditId ? 'Log Today\'s Progress' : 'LeetCode Entry'}</div>
    <div id="lc-form-wrap">${lcFormHTML(lcEditId)}</div>
  </div>

  <div class="lc-section-lbl">🏅 Member Rankings — ${sorted.length} members</div>
  <div class="lc-grid">
...
function lcFormHTML(editingMemberId){
  ...
  const isInitial = !existingEntry; // no entry yet → show full initial form
  ...
  if(isInitial){
    // ── FIRST TIME: full setup form ──
    return memberDisplay + `
    <div class="info-pill ip-info" style="margin-bottom:14px">📋 First time setup — enter your current LeetCode stats as a baseline</div>
    <div class="fg3">
      <div class="field"><label>LeetCode Handle 🔒 <span style="font-size:.65rem;color:var(--t3);font-weight:400;text-transform:none;letter-spacing:0">(fixed after save)</span></label><input id="lc-handle" placeholder="john_doe123"></div>
      <div class="field"><label>Contest Rating</label><input id="lc-rating" type="number" min="0" placeholder="1450"></div>
      <div class="field"><label>Easy Solved</label><input id="lc-easy" type="number" min="0" placeholder="0"></div>
      <div class="field"><label>Medium Solved</label><input id="lc-medium" type="number" min="0" placeholder="0"></div>
      <div class="field"><label>Hard Solved</label><input id="lc-hard" type="number" min="0" placeholder="0"></div>
      <div class="field"><label>Daily Streak (days)</label><input id="lc-streak" type="number" min="0" placeholder="0"></div>
      <div class="field"><label>Target (total problems)</label><input id="lc-target" type="number" min="0" placeholder="500"></div>
    </div>
    <div class="act">
      <button class="btn btn-lc-baseline" onclick="saveLeetCode()">💾 Save Baseline</button>
    </div>`;
  } else {
    // ── DAILY LOG: only today's additions ──
    const total = (existingEntry.easy||0)+(existingEntry.medium||0)+(existingEntry.hard||0);
    return memberDisplay + `
    <div class="info-pill ip-ok" style="margin-bottom:14px">📈 Current total: <strong>${total}</strong> solved &nbsp;·&nbsp; @${existingEntry.handle} &nbsp;·&nbsp; 🔥 ${existingEntry.streak||0}-day streak</div>
    <div style="font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.07em;color:var(--acc4);margin-bottom:10px">➕ Log Today's Progress</div>
    <div class="fg3">
      <div class="field"><label>Easy solved today</label><input id="lc-daily-easy" type="number" min="0" placeholder="0"></div>
      <div class="field"><label>Medium solved today</label><input id="lc-daily-medium" type="number" min="0" placeholder="0"></div>
      <div class="field"><label>Hard solved today</label><input id="lc-daily-hard" type="number" min="0" placeholder="0"></div>
      <div class="field"><label>Update streak (days)</label><input id="lc-streak" type="number" min="0" placeholder="${existingEntry.streak||0}" value="${existingEntry.streak||0}"></div>
      <div class="field"><label>Note (optional)</label><input id="lc-daily-note" placeholder="e.g. Solved Two Sum, DP practice…"></div>
    </div>
    <div class="act">
      <button class="btn btn-lc-save" onclick="saveDailyLog()">🔄 Add to Summary</button>
...
function lcPopulateForm(memberId){
  // Only used for initial baseline edit — daily mode pre-fills via lcFormHTML
  const entry = leetcodeStats.find(x=>sid(x.memberId)===sid(memberId));
  if(!entry) return;
  setV('lc-rating',  entry.rating   || '');
  setV('lc-easy',    entry.easy     || '');
  setV('lc-medium',  entry.medium   || '');
  setV('lc-hard',    entry.hard     || '');
  setV('lc-streak',  entry.streak   || '');
...
async function saveLeetCode(){
  // Saves the INITIAL BASELINE entry (first time only)
  ...
  const handle = v('lc-handle');
  if(!handle){ toast('LeetCode handle is required','err'); return; }

  const easy   = parseInt(v('lc-easy'))   || 0;
  const medium = parseInt(v('lc-medium')) || 0;
  const hard   = parseInt(v('lc-hard'))   || 0;
  ...
  const entry = {
    ...
    handle,
    rating:     parseInt(v('lc-rating'))  || 0,
    easy, medium, hard,
    streak:     parseInt(v('lc-streak')) || 0,
  ...
  toast(`✅ Baseline saved for ${member.name}! Use "Log Today" to add daily progress.`);
  lcEditId = null;
  renderLeetCode();
}

async function saveDailyLog(){
  // Adds today's solved count ON TOP of existing totals
  const sel = document.getElementById('lc-member-sel');
  const memberId = sel ? sel.value : String(CU.id);
  if(!memberId){ toast('Select a member','err'); return; }
  const canEditAll = ['captain','vice','manager'].includes(CU.role);
  if(!canEditAll && sid(memberId) !== sid(CU.id)){ toast('You can only update your own entry','err'); return; }
  const member = members.find(m=>sid(m.id)===sid(memberId));
  if(!member){ toast('Member not found','err'); return; }

  const existing = leetcodeStats.find(x=>sid(x.memberId)===sid(memberId));
  if(!existing){ toast('No baseline entry found — save your initial stats first','err'); return; }

  const addEasy   = parseInt(document.getElementById('lc-daily-easy')?.value)   || 0;
  const addMedium = parseInt(document.getElementById('lc-daily-medium')?.value) || 0;
  const addHard   = parseInt(document.getElementById('lc-daily-hard')?.value)   || 0;
  const note      = document.getElementById('lc-daily-note')?.value?.trim() || '';
  const newStreak = parseInt(document.getElementById('lc-streak')?.value) || existing.streak || 0;
  const newRating = existing.rating || 0;

  if(addEasy===0 && addMedium===0 && addHard===0 && !note){
    toast('Enter at least 1 problem solved or a note','err'); return;
  }

  // Build daily log entry
  const logEntry = {
    date: today(),
    easy: addEasy, medium: addMedium, hard: addHard,
    total: addEasy + addMedium + addHard,
    note, streak: newStreak
  };

  // Update cumulative totals
  existing.easy   = (existing.easy   || 0) + addEasy;
  existing.medium = (existing.medium || 0) + addMedium;
  existing.hard   = (existing.hard   || 0) + addHard;
  existing.streak = newStreak;
  existing.rating = newRating;
  existing.dailyLog = [...(existing.dailyLog || []), logEntry];
  existing.updatedOn = today();
  existing.updatedBy = CU.name;

  await saveDoc('leetcodeStats', existing);
  const added = addEasy + addMedium + addHard;
  toast(`📈 +${added} problems added to ${member.name}'s summary!`);
  lcEditId = null;
  renderLeetCode();
}
  ...
  return `<div class="lc-card">
    <div class="lc-header">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div class="lc-handle">${entry&&entry.handle ? '@'+entry.handle : '<span style="color:var(--t4)">No entry yet</span>'}</div>
      </div>
      <div>${rankIcon}</div>
    </div>
    ...
    <div class="lc-updated" style="margin-top:8px">Updated: ${entry.updatedOn} by ${entry.updatedBy}</div>
    ` : `<div class="lc-no-entry">No entry yet — tap Add to get started</div>`}

    ${canEdit && !entry ? `<div class="lc-actions">
      <button class="btn btn-s btn-xs" onclick="lcEditEntry(${m.id})">➕ Add Baseline</button>
    </div>` : ''}
    ${entry&&canEditAll ? `<div class="lc-actions">
      <button class="btn btn-d btn-xs" onclick="lcDeleteEntry(${m.id})">🗑</button>
    </div>` : ''}
  </div>`;
}
...
function renderAdmin(){
  ...
  const teamInfoHTML = `
  <div class="card" style="margin-bottom:18px">
    <div class="ct"><span class="cd" style="background:var(--acc2)"></span>Team Info — Invite Members</div>
    <div class="info-pill" style="margin-bottom:14px">Share the <strong>Team Code</strong> and <strong>Passcode</strong> below with new teammates — they'll use "Join Existing Team" on the TeamSync start screen to get in.</div>
    <div style="display:flex;gap:18px;flex-wrap:wrap;margin-bottom:14px">
      <div style="flex:1;min-width:160px">
        <div style="font-size:.72rem;color:var(--t3);text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Team Name</div>
        <div style="font-weight:700">${escHTML(TEAM_NAME||'')}</div>
      </div>
      <div style="flex:1;min-width:140px">
        <div style="font-size:.72rem;color:var(--t3);text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Team Code</div>
        <div style="font-weight:800;font-family:var(--mono);letter-spacing:.12em;color:var(--acc)">${TEAM_CODE||''}</div>
      </div>
      <div style="flex:1;min-width:160px">
        <div style="font-size:.72rem;color:var(--t3);text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">Passcode</div>
        <div style="display:flex;align-items:center;gap:8px">
          <span id="admin-passcode-val" style="font-weight:800;font-family:var(--mono);letter-spacing:.12em">••••••</span>
          <button class="btn btn-d btn-xs" id="admin-passcode-btn" onclick="toggleAdminPasscode()">Show</button>
        </div>
      </div>
    </div>
    <div class="act"><button class="btn btn-d btn-sm" onclick="changeTeam()">🔀 Switch Team</button></div>
  </div>`;
...
function toggleAdminPasscode(){
  const el=document.getElementById('admin-passcode-val');
  const btn=document.getElementById('admin-passcode-btn');
  if(!el||!btn)return;
  if(btn.textContent==='Show'){ el.textContent=TEAM_PASSCODE||''; btn.textContent='Hide'; }
